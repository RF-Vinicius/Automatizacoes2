...
def renomear_revisao_docs(caminho_doc):
    ...
    for nome_arquivo in os.listdir(caminho_doc):
        ...
        if (re.search(padrao01, listNome[-1])) and not (re.search(padrao03, nome_base)):
            novo_nome = listNome
            novo_nome[-1] = novo_nome[-1].replace("R","E")   
            novo_nome_documento = "-".join(novo_nome) + extensao
            print(novo_nome_documento)
        
        elif re.search(padrao02, "".join(listNome[-2:])):
            novo_nome = listNome
            novo_nome_documento = "-".join(novo_nome[:-2]) + "-" + "".join(novo_nome[-2:]) + extensao
            print(novo_nome_documento)     
        else:
        ...
        try:
            ...
            print(f"Renomeado: '{nome_arquivo}' -> '{novo_nome_documento}'")
